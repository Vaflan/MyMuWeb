...
function clear_logs($name)
{
   ...
   fwrite ($fp, $new_data);
	echo "Log $name SuccessFully Deleted!";
}
   ...
   // START LOGIN
	if(isset($_POST["admin_login"])) {
		...
		if(($account == NULL) || ($password == NULL) || ($securitycode == NULL)) {die("<center><b>Fatal ErroR! by Vaflan</b></center>");}

		if($mmw[md5] == 1) {$login_sql = mssql_query("SELECT memb___id,admin FROM dbo.MEMB_INFO WHERE memb___id='$account' AND memb__pwd=[dbo].[fn_md5]('$password','$account')");}
		elseif ($mmw[md5] == 0) {$login_sql = mssql_query("SELECT memb___id,admin FROM dbo.MEMB_INFO WHERE memb___id='$account' AND memb__pwd='$password'");}
		...
		if($login_row[0] == $account && $login_row[1] < $mmw[min_level_to_ap]) {
			echo '<script language="Javascript">alert("Ups! '.$login_row[0].' You Can\'t Enter in Here!"); window.location=".";</script>';
			}
	...
	// Logout
		if(isset($_POST["admin_logout"])) { 
			...
			echo '<script language="Javascript">alert("You Have Logged Out From Your Admin Control Panel, Press Ok To Go To The Main WebSite!"); window.location=".";</script>';
			}
	// CHECK LOGIN
		if (isset($_SESSION['a_admin_security'],$_SESSION['a_admin_pass'],$_SESSION['a_admin_login'],$_SESSION['a_admin_level'])){
			...
			if($mmw[md5] == 1) {$check_sql = mssql_query("SELECT memb___id,admin FROM dbo.MEMB_INFO WHERE memb___id='$admin' AND memb__pwd=[dbo].[fn_md5]('$pass','$admin')");}
			elseif ($mmw[md5] == 0) {$check_sql = mssql_query("SELECT memb___id,admin FROM dbo.MEMB_INFO WHERE memb___id='$admin' AND memb__pwd='$pass'");}
			...
			if($mmw[admin_securitycode] != $code || $admin != $check_row[0] || $level != $check_row[1] || $level < $mmw[min_level_to_ap]) {
				...
				unset($_SESSION["a_admin_security"]);
				unset($_SESSION["a_admin_level"]);
				echo '<script language="Javascript">alert("Dear '.$_SERVER["REMOTE_ADDR"].', Don\'t Try Fuckin Things!");window.location="index.php";</script>';
				}
			}
   // END LOGIN
...
{     require("config.php");
      if (empty($post_name) ||  empty($post_version) || empty($post_experience) || empty($post_drops) || empty($post_gsport) || empty($post_serverip) || empty($post_order) ||  empty($post_type)){
	         echo "Error: Some Fields Were Left Blank!<br><a href='javascript:history.go(-1)'>Go Back.</a>";}
                 elseif (ereg('[^0-9]', $post_order)){echo "Error: Please Use Only Numbers At Displaying Order!  <br><a href='javascript:history.go(-1)'>Go Back.</a>"; }
               else{
                      ...
                      echo "$post_name Server SuccessFully Added!";
...
{         require("config.php");
          if(empty($name) || empty($version) || empty($experience) || empty($drops) || empty($server_type) || empty($gsport) || empty($serverip) || empty($order) || empty($old_name)){
                   echo "Error: Some Fields Were Left Blank!<br><a href='javascript:history.go(-1)'>Go Back.</a>";}
                                ...
                                echo "$old_name Server SuccessFully Edited!";
...
{     require("config.php");
      if (empty($post_server_name_delete)){echo "Error: Some Fields Were Left Blank!<br><a href='javascript:history.go(-1)'>Go Back.</a>";}
                else{
                        mssql_query("Delete from MMW_servers where name='$post_server_name_delete'");
                        echo "$post_server_name_delete Server SuccessFully Deleted!";
...
function add_new_news($news_title,$news_autor,$news_category,$news_eng,$news_rus)
{
           require("config.php");
           $date = time();
               if (empty($news_title) ||  empty($news_autor) || empty($news_category)){
                      echo "Error: Some Fields Were Left Blank!<br><a href='javascript:history.go(-1)'>Go Back.</a>";}
                           else{
                                  mssql_query("INSERT INTO MMW_news(news_title,news_autor,news_category,news_date,news_eng,news_rus,news_id) VALUES ('$_POST[news_title]','$_POST[news_autor]','$_POST[category]','$date','$news_eng','$news_rus','$mmw[news_id]')");
                                  echo "News SuccessFully Added!";

                                  $log_dat = "News: $_POST[news_title] Has Been <font color=#FF0000>Added</font> Author: $_POST[news_autor]";
...
{
        require("config.php");
           if (empty($news_id)){echo "Error: Some Fields Were Left Blank!<br><a href='javascript:history.go(-1)'>Go Back.</a>"; }
                else{
...
function edit_news($news_title,$news_autor,$news_cateogry,$news_id,$news_eng,$news_rus)
{
        ...
        $date = date('d-m-Y H:i');
          if (empty($news_title) ||  empty($news_autor) || empty($news_cateogry) ||  empty($news_id)){
	          echo "$news_title,$news_autor,$news_cateogry,$news_date,$news_time,$news_id Error: Some Fields Were Left Blank!<br><a href='javascript:history.go(-1)'>Go Back.</a>";}
                    else{
...
function edit_character() {
     ...
     $get_acc_chr = mssql_fetch_row($get_chr);
          ...
          if (empty($post_character) ||  empty($post_level) || empty($post_strength) || empty($post_agility) || empty($post_vitality) || empty($post_energy)){
                  echo "Error: Some Fields Were Left Blank!<br><a href='javascript:history.go(-1)'>Go Back.</a>";}
                   elseif ($name_check <= 0){ echo "Error: Character $post_character Doesn't Exist!<br><a href='javascript:history.go(-1)'>Go Back.</a>"; }
                    elseif ($oc_row[0] != 0 && $get_acc_chr[0]==$get_account_done[1]){ echo "Error: Character $post_character Must Be Logged Off!<br><a href='javascript:history.go(-1)'>Go Back.</a>"; }
                     elseif (ereg('[^0-9]', $post_level)){echo "Error: Please Use Only Numbers At Level!<br><a href='javascript:history.go(-1)'>Go Back.</a>"; }
                      elseif (ereg('[^0-9]', $post_reset)){echo "Error: Please Use Only Numbers At Reset!<br><a href='javascript:history.go(-1)'>Go Back.</a>"; }
                       elseif (ereg('[^0-9]', $post_zen)){echo "Error: Please Use Only Numbers At Zen!<br><a href='javascript:history.go(-1)'>Go Back.</a>"; }
                        elseif (ereg('[^0-9]', $post_strength)){echo "Error: Please Use Only Numbers At Strength!<br><a href='javascript:history.go(-1)'>Go Back.</a>"; }
                         elseif (ereg('[^0-9]', $post_agility)){echo "Error: Please Use Only Numbers At Agiltiy!<br><a href='javascript:history.go(-1)'>Go Back.</a>"; }
                          elseif (ereg('[^0-9]', $post_vitality)){echo "Error: Please Use Only Numbers At Vitality!<br><a href='javascript:history.go(-1)'>Go Back.</a>"; }
                           elseif (ereg('[^0-9]', $post_energy)){echo "Error: Please Use Only Numbers At Energy!<br><a href='javascript:history.go(-1)'>Go Back.</a>"; }
                            elseif (ereg('[^0-9]', $post_command)){echo "Error: Please Use Only Numbers At Command!<br><a href='javascript:history.go(-1)'>Go Back.</a>"; }
                             elseif (ereg('[^0-9]', $post_leveluppoint)){echo "Error: Please Use Only Numbers At Level Up Point!<br><a href='javascript:history.go(-1)'>Go Back.</a>"; }
                              elseif (ereg('[^0-9]', $post_pklevel)){echo "Error: Please Use Only Numbers At PK Level!<br><a href='javascript:history.go(-1)'>Go Back.</a>"; }
                               elseif (ereg('[^0-9]', $post_pktime)){echo "Error: Please Use Only Numbers At PK Time!<br><a href='javascript:history.go(-1)'>Go Back.</a>"; }
                                elseif (ereg('[^0-9]', $post_mapnumber)){echo "Error: Please Use Only Numbers At Map Number!<br><a href='javascript:history.go(-1)'>Go Back.</a>"; }
                                 elseif (ereg('[^0-9]', $post_mapposx)){echo "Error: Please Use Only Numbers At Map x!<br><a href='javascript:history.go(-1)'>Go Back.</a>"; }
                                  elseif (ereg('[^0-9]', $post_mapposy)){echo "Error: Please Use Only Numbers At Map y!<br><a href='javascript:history.go(-1)'>Go Back.</a>"; }
                             else{
                                     ...
                                     echo "Character $post_character SuccessFully Edited!";
...
function edit_account($post_account,$post_pwd,$post_mode,$post_email,$post_squestion,$post_sanswer,$post_unblock_time,$post_block_date,$post_blocked_by,$post_block_reason)
{
        require("config.php");
        ...
        $row2 = mssql_fetch_row($sql_online_check);
           if (empty($post_account) || empty($post_email) || empty($post_squestion) || empty($post_sanswer)){echo "<img src=./images/warning.gif> Error: Some Fields Were Left Blank!  <br><a href='javascript:history.go(-1)'>Go Back.</a>";}
                   elseif ($account_check <= 0){echo "Error: Account $post_account Doesn't Exist!<br><a href='javascript:history.go(-1)'>Go Back.</a>"; }
                    elseif ($row2[0] != 0){echo "Error: Account $post_account Must Be Logged Off!<br><a href='javascript:history.go(-1)'>Go Back.</a>"; }
                     else{
				...
				$block_menu = $block_menu . "[blocked_by]='$post_blocked_by',[block_reason]='$post_block_reason',";
                            if($mmw['md5']==1){$sql_script_edit_account = "Update memb_info set $passmd5 $block_menu [bloc_code]='$post_mode',[mail_addr]='$post_email',[fpas_ques]='$post_squestion',[fpas_answ]='$post_sanswer' where memb___id='$post_account'";}
                                 elseif($mmw['md5']==0){$sql_script_edit_account = "Update memb_info set $passdef $block_menu [bloc_code]='$post_mode',[mail_addr]='$post_email',[fpas_ques]='$post_squestion',[fpas_answ]='$post_sanswer' where memb___id='$post_account'";}

                                   mssql_query($sql_script_edit_account);
                                   echo "Account $post_account SuccessFully Edited!";

                                   $log_dat = "Account $_POST[account] Has Been <font color=#FF0000>Edited</font> with the next->New Password:$_POST[new_pwd]|E-mail:$_POST[email]|Secret Question:$_POST[squestion]|Secret Answer:$_POST[sanswer]";
...
function edit_acc_wh($post_account,$post_warehouse,$post_extwarehouse)
{
        ...
        $sql_account_check = mssql_query("SELECT memb___id FROM memb_info WHERE memb___id='$post_account'"); 
        $account_check = mssql_num_rows($sql_account_check); 
           if (empty($post_account) || $post_warehouse<0 || $post_extwarehouse<0){echo "Error: Some Fields Were Left Blank!  <br><a href='javascript:history.go(-1)'>Go Back.</a>";}
                   elseif ($account_check <= 0){echo "Error: Account $post_account Doesn't Exist!<br><a href='javascript:history.go(-1)'>Go Back.</a>"; }
                     else{
                                   mssql_query("Update warehouse set [Money]='$post_warehouse',[extMoney]='$post_extwarehouse' where AccountID='$post_account'");
                                   echo "Acc Ware House $post_account SuccessFully Edited!";
...
{        require("config.php");
         if (empty($account)) {
	      echo "Error: Some Fields Were Left Blank!<br><a href='javascript:history.go(-1)'>Go Back.</a>";}
			...
			if($no_error == 1) {
			echo "Account $account SuccessFully Deleted!";
...
{
        require("config.php");
           if (empty($sql_query)){echo "Error: Some Fields Were Left Blank!  <br><a href='javascript:history.go(-1)'>Go Back.</a>";}
             else{
		   if(mssql_query("$sql_query"))
			{echo "SQL Query SuccessFully Edited!";}
		   else
			{echo "<b>Error:</b> $sql_query";}
