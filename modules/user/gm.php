...
?>
  ...
  <table class="sort-table" border="0" cellspacing="0" cellpadding="0" align="center">
    <tr>
      ...
      <td><b><?php echo $_SESSION['mmw_status'];?> (<?php echo $mmw['status_rules'][$_SESSION['mmw_status']]['name']; if($_SESSION['mmw_status'] < 1) {echo 'HACKER DETECTED!';}?>)</b></td>
    </tr>
    <tr>
      <td align="right">Security Code:</td>
      <td><b><?php echo $mmw['security_code'];?></b></td>
    </tr>
    <tr>
...
if($mmw['status_rules'][$_SESSION['mmw_status']]['hex_wh'] > 0) {
 echo "<center><b>HEX Ware House Can Edit!</b></center>" . $rowbr;
 //HEX WH
 mssql_query("declare @vault varbinary(1920); set @vault=(SELECT Items FROM warehouse where AccountId='".$account."'); print @vault;");
 $vault = substr(mssql_get_last_message(),2);
 $result = mssql_query("SELECT Money,extMoney FROM warehouse WHERE accountid='".$account."'");
...
<form name="edit_wh" method="post" action="">
  <table class="sort-table" align="center" border="0" cellpadding="0" cellspacing="0" width="90%">           
	<tr>
          <td align="center" colspan="2"><textarea name="hex_wh" rows="12" style="width: 100%;"><?php echo $vault;?></textarea></td>
	</tr>
	<tr>
          <td align="center"><?php echo mmw_lang_ware_house;?>: <input name='Money' type='text' value='<?php echo $row[0];?>' size='14'></td>
          <td align="center"><?php echo mmw_lang_extra_ware_house;?>: <input name='extMoney' type='text' value='<?php echo $row[1];?>' size='14'></td>
	</tr>
	<tr>
...
if($mmw['status_rules'][$_SESSION['mmw_status']]['gm_msg'] > 0) {
...
if($mmw['status_rules'][$_SESSION['mmw_status']]['gm_block'] > 0) {
...
<center><a href="http://www.mymuweb.ru/"><i>Thank Vaflan For This MMW!</i></a><br></center>