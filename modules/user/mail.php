...
if(isset($_POST["view_msg_inbox"])) {
 ...
 if($view_msg_row[1]!='Guard') {
  $view_msg_re ="<form action='?op=user&u=mail&to=$view_msg_row[1]' method='post' name='view_msg_re'>
	<input name='Reply' type='submit' value='".mmw_lang_reply."'>
	<input name='send_msg_subject' type='hidden' value='".win_to_utf($view_msg_row[2])."'>
  </form>";
 }

 $query = "declare @vault varbinary(13); SET @vault=(SELECT Photo FROM T_FriendMail where GUID='$char_guid' and MemoIndex='$view_msg_inbox'); print @vault;";
...
?>
  <table width='300' class='sort-table' border='0' cellspacing='0' cellpadding='0' align='center'>
    <tr>
      <td align='right' width='30'><?echo mmw_lang_msg_from;?>:</td>
      <td><a href='?op=character&character=<?echo $view_msg_row[1];?>'><?echo $view_msg_row[1];?></a> <?echo $view_msg_re;?></td>
    </tr>
    <tr>
      <td align='right'><?echo mmw_lang_title;?>:</td>
      <td><?echo win_to_utf($view_msg_row[2]);?></td>
    </tr>
    <tr>
      <td align='right'><?echo mmw_lang_text;?>:</td>
      <td><?echo win_to_utf($view_msg_row[4]);?></td>
    </tr>
    <tr>
      <td align='right'><?echo mmw_lang_date;?>:</td>
      <td><?echo time_format($view_msg_row[3],"d M Y, H:i");?></td>
    </tr>
  </table>
<?
 if($view_msg_row[5]==0){
...
// List Message
if($inbox_msg = @mssql_query("SELECT MemoIndex,FriendName,Subject,wDate,bRead,photo FROM T_FriendMail WHERE GUID='$char_guid' ORDER BY MemoIndex DESC")) {$inbox_msg_num = @mssql_num_rows($inbox_msg);}
else {$inbox_msg_num = 0; echo $die_start . mmw_lang_error_mail_table . $die_end . $rowbr;}
?>
<table class="sort-table" height="0" border="0" cellpadding="4" cellspacing="0" align="center">                
	<thead><tr>
	<td width="50"><?echo mmw_lang_msg_from;?></td>
	<td><?echo mmw_lang_title;?></td>
	<td width=112><?echo mmw_lang_date;?></td>
	<td width=30 aling=center><?echo mmw_lang_view;?></td>
	<td width=40 aling=center><?echo mmw_lang_delete;?></td>
	<td width=20 aling=center><?echo mmw_lang_status;?></td>
</tr></thead>
<?
if($inbox_msg_num > 0) {
 for($i=0;$i < $inbox_msg_num;++$i) {
  ...
  $msg_table_view ="<form action='' method='post' name='view_msg'>
	<input name='View' type='submit' value='".mmw_lang_view."'>
	<input name='view_msg_inbox' type='hidden' value='$select_msg[0]'>
  </form>";

  $msg_table_delete ="<form action='' method='post' name='delete_msg_inbox'>
