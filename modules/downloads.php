...
$get_links = mssql_query("SELECT link_name,link_address,link_description,link_size,link_date from MMW_Links order by link_date desc");

for($i=0;$i < mssql_num_rows($get_links);++$i) {
$row = mssql_fetch_row($get_links);

echo	"<table cellpadding='1' width='100%' class='aBlock'><tr>
	<td><a href='$row[1]' target='_blank'><b><big>$row[0]</big></b></a></td><td align='right'>".mmw_lang_date.": $row[4]</td></tr>
