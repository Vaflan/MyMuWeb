...
var x; x = (ex.getTime() - now.getTime())/1000;   
...
if (endmin<10) {endmin = "0"+endmin;} if (endsec<10) {endsec = "0"+endsec;}
if (endhour>0) {endtime = endhour + ":" + endmin + ":" + endsec;}
else {endtime = endmin + ":" + endsec;} 
if (endhour<0) {endtime = "Close";}
if (endhour==-1 && endmin>54) {endtime = "Open";}
document.getElementById("dstime").innerHTML = endtime;
setTimeout("dsetime()", 1000);//script_by_vaflan
}
...
setTimeout("bcetime()", 1000);//script_by_vaflan
}
...
setTimeout("ccetime()", 1000);//script_by_vaflan
}
